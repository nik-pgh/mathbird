...
3. **Vendor SDKs imported only inside factory branches.** `from livekit.plugins import deepgram` lives in `app/agent/providers/stt.py` and *nowhere else*. Same rule for `openai`, `cartesia`, `boto3`, etc. Lazy imports keep startup fast and let users skip plugins they don't need.
...
| Real RAG implementation | New module under `app/rag/` implementing `Retriever`; return from `get_retriever()` |
