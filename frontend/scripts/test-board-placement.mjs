...
assert.match(workspaceSource, /import SessionBoardTools from "\.\/SessionBoardTools"/);
assert.match(workspaceSource, /import \{ useRegisterSessionToolbar \} from "\.\/SessionToolbarContext"/);
...
assert.match(workspaceSource, /useRegisterSessionToolbar\(sessionToolbar\)/);
assert.match(workspaceSource, /<SessionBoardTools[\s\S]*?inkTool=\{state\.ink\.tool\}[\s\S]*?inkColor=\{state\.ink\.color\}[\s\S]*?canUndoInk=\{canChangeActiveInk\}[\s\S]*?canClearInk=\{canChangeActiveInk\}[\s\S]*?onInkToolChange=\{setInkTool\}[\s\S]*?onInkColorChange=\{setInkColor\}[\s\S]*?onInkUndo=\{undoActiveInk\}[\s\S]*?onInkClear=\{clearActiveInk\}/);
assert.doesNotMatch(workspaceSource, /<SessionBoardRibbon/);
...
const sessionBoardToolsPath = new URL("../src/components/session/SessionBoardTools.tsx", import.meta.url);
const sessionBoardToolsSource = fs.readFileSync(sessionBoardToolsPath, "utf8");
assert.match(sessionBoardToolsSource, /import BoardInkToolbar from "\.\/BoardInkToolbar"/);
assert.match(sessionBoardToolsSource, /aria-label="Add student card"/);
assert.match(sessionBoardToolsSource, /aria-label="Add sticky note"/);
assert.match(sessionBoardToolsSource, /aria-label=\{textbookToggleLabel\}/);
assert.match(sessionBoardToolsSource, /className=\{`topbar-board-tools-button/);
assert.match(sessionBoardToolsSource, /CANVAS_WHEEL_IGNORE_ATTR/);

const sessionTopbarPath = new URL("../src/components/session/SessionTopbar.tsx", import.meta.url);
const sessionTopbarSource = fs.readFileSync(sessionTopbarPath, "utf8");
assert.match(sessionTopbarSource, /topbar-session-center/);
assert.match(sessionTopbarSource, /useSessionToolbarContent/);
...
assert.match(textbookOverlaySource, /zoom=page-width/);
assert.match(textbookOverlaySource, /CANVAS_WHEEL_IGNORE_ATTR/);
assert.match(textbookOverlaySource, /onClose/);
...
assert.match(globalCss, /\.topbar--session\s*\{/s);
assert.match(globalCss, /\.topbar-session-center\s*\{/s);
assert.match(globalCss, /\.topbar-board-tools\s*\{/s);
assert.match(globalCss, /\.topbar-board-tools-button\s*\{/s);
assert.match(globalCss, /\.topbar-board-tools-button\.is-active\s*\{/s);
assert.match(sessionCss, /\.shared-workspace\s*\{[^}]*grid-template-rows:\s*minmax\(0,\s*1fr\) auto;/s);
assert.doesNotMatch(sessionCss, /\.session-board-ribbon\s*\{/s);
...
assert.match(sessionCss, /\.board-ink-toolbar\s*\{[^}]*flex-direction:\s*row;/s);
...
assert.match(sessionCss, /\.board-ink-divider\s*\{[^}]*width:\s*1px;[^}]*height:\s*22px;/s);
assert.match(sessionCss, /\.ink-color-swatch\s*\{/s);
assert.doesNotMatch(sessionCss, /\.textbook-overlay-head/);
assert.doesNotMatch(sessionCss, /\.board-top-actions\s*\{/s);
...
assert.match(sessionCss, /\.textbook-overlay-close\s*\{/s);
